...
public class TeacherCommandHandler {
    ...
    public void sendHelp(Long chatId) {
        String helpText = """
                ...
                ➕ Создать консультацию - пошаговое создание новой консультации
                📋 Просмотреть запросы - запросы студентов на консультации
                👤 Профиль - редактирование имени и фамилии
                
                В разделе "📅 Мои консультации":
                • Просмотр списка консультаций (введите №... для выбора)
                • После выбора конкретной консультации можно:
                 * Редактирование (название, дата/время, вместимость, автозакрытие)
                 * Управление статусом (открыть/закрыть вручную)
                 * Отмена консультации с указанием причины
                 * Просмотр списка записанных студентов
                
                В разделе "📋 Просмотреть запросы":
                • Просмотр запросов от студентов (введите №... для выбора)
                • После выбора конкретного запроса можно:
                 * Принятие запроса - создание консультации на его основе
                   - Все заинтересованные студенты автоматически записываются
                
                🤖 Автоматические процессы:
                • Автозакрытие при заполнении мест
                • Автооткрытие при освобождении мест
                • Закрытие прошедших консультаций (ежедневно 00:00)
                • Удаление консультаций старше 30 дней (ежедневно 00:05)
                """;
    ...
    private void processEditCapacity(Long chatId, String capacityInput) {
        ...
        consultationRepository.findById(consultationId).ifPresentOrElse(
                consultation -> {
                    ...
                    // Уведомляем подписчиков только если консультация автоматически открылась (статус изменился на OPEN)
                    if (statusChanged && consultation.getStatus() == ConsultationStatus.OPEN) {
                        notificationService.notifySubscribersAvailableSpots(consultation.getId(), null);
                    }
